...
bool QQueue<T>::enqueue(int tid, int opn, T v)
{
	...
	while (true)
	{
		...
		{
			Node *head_next_next = head_next->next.load(); //CLP ADDED, We want to keep the first node as a sentinel 
			//Remove the pending dequeue operation at the head
			//if (_head[Index].compare_exchange_weak(head, head_next))
			if (head->next.compare_exchange_weak(head_next, head_next_next))
			{
				if (head->next.load() == nullptr)
				{
					_tail[Index].compare_exchange_weak(tail, head);
				}
				//return true;
				return false; //CLP ADDED
			}
		}	
	}
}
...
bool QQueue<T>::dequeue(int tid, int opn, T& v)
{
	...
	while (true)
	{
		...
		else
		{
			Node *head_next_next = head_next->next.load(); //CLP ADDED, We want to keep the first node as a sentinel 
			//v = head->value();
			v = head_next->value(); //head_next is actually the head of the queue
			//Dequeue as normal
			//if (_head[Index].compare_exchange_weak(head, head_next))
			if (head->next.compare_exchange_weak(head_next, head_next_next))
			{
