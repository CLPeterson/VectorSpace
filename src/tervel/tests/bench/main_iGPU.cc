...
cl::Buffer* buffer_sum_ptr;
cl::Buffer* buffer_sum_f_ptr;
cl::Buffer* buffer_sum_r_ptr;
cl::Buffer* buffer_D_ptr;
cl::CommandQueue* device_queue_ptr;
...
//void verify_checkpoint(std::map<long int,Method,bool(*)(long int,long int)>& map_methods, Item* vector_items[], std::map<long int,Method,bool(*)(long int,long int)>::iterator& it_start, long unsigned int& count_iterated, long int min, bool reset_it_start, std::map<long int,Block,bool(*)(long int,long int)>& map_block, double* sum, double* sum_f, double* sum_r)
void verify_checkpoint(std::map<long int,Method,bool(*)(long int,long int)>& map_methods, Item* vector_items[], std::map<long int,Method,bool(*)(long int,long int)>::iterator& it_start, long unsigned int& count_iterated, long int min, bool reset_it_start, std::map<long int,Block,bool(*)(long int,long int)>& map_block, double* sum, double* sum_f)
{
	if(!map_methods.empty())
	{
		...
		for (; it!=map_methods.end(); ++it)
		{
			...
			sum[index] = vec_item->sum;
			//sum_r[index] = vec_item->sum_r;
		}
		if(reset_it_start)
		{
			--it_start;
		...
		while(!stack_consumer.empty())
		{
			...
			while(!it_top->promote_items.empty())
			{
				...
				sum[index] = vec_promote_item->sum;
		...
		//device_queue.enqueueWriteBuffer(buffer_sum,CL_TRUE,0,sizeof(double)*(TEST_SIZE*NUM_THRDS+1),sum);
		device_queue_ptr->enqueueWriteBuffer(*buffer_sum_ptr,CL_TRUE,0,sizeof(double)*(TEST_SIZE*NUM_THRDS+1),sum);
		//device_queue.enqueueWriteBuffer(buffer_sum_f,CL_TRUE,0,sizeof(double)*(TEST_SIZE*NUM_THRDS+1),sum_f);
		device_queue_ptr->enqueueWriteBuffer(*buffer_sum_f_ptr,CL_TRUE,0,sizeof(double)*(TEST_SIZE*NUM_THRDS+1),sum_f);
		//device_queue.enqueueWriteBuffer(buffer_sum_r,CL_TRUE,0,sizeof(double)*(TEST_SIZE*NUM_THRDS+1),sum_r);
		//device_queue_ptr->enqueueWriteBuffer(*buffer_sum_r_ptr,CL_TRUE,0,sizeof(double)*(TEST_SIZE*NUM_THRDS+1),sum_r);
		device_queue_ptr->enqueueWriteBuffer(*buffer_D_ptr,CL_TRUE,0,sizeof(unsigned int)*1,D);
		
		//cl::Kernel simple_add(program, "simple_add");
		cl::Kernel simple_add(*program_ptr, "simple_add");
		//simple_add.setArg(0, buffer_sum);
		simple_add.setArg(0, *buffer_sum_ptr);
		//simple_add.setArg(1, buffer_sum_f);
		simple_add.setArg(1, *buffer_sum_f_ptr);
		//simple_add.setArg(2, buffer_sum_r);
		//simple_add.setArg(2, *buffer_sum_r_ptr);
		//simple_add.setArg(3, buffer_D);
		//simple_add.setArg(3, *buffer_D_ptr);
		simple_add.setArg(2, *buffer_D_ptr);
		//device_queue.enqueueNDRangeKernel(simple_add,cl::NullRange,cl::NDRange(10),cl::NullRange);
		device_queue_ptr->enqueueNDRangeKernel(simple_add,cl::NullRange,cl::NDRange(10),cl::NullRange);
		
		//unsigned long int D[10];
		//read result C from the device to array C
		//device_queue.enqueueReadBuffer(buffer_D,CL_TRUE,0,sizeof(unsigned long int)*1,D);
		device_queue_ptr->enqueueReadBuffer(*buffer_D_ptr,CL_TRUE,0,sizeof(unsigned int)*1,D);
...
void verify()
{
	...
	double sum[TEST_SIZE*NUM_THRDS+1] = {};
	double sum_f[TEST_SIZE*NUM_THRDS+1] = {};
	//double sum_r[TEST_SIZE*NUM_THRDS+1] = {};
	...
	while(!stop)
	{
		...
		//verify_checkpoint(map_methods, vector_items, it_start, count_iterated, min, true, map_block, sum, sum_f, sum_r);
		verify_checkpoint(map_methods, vector_items, it_start, count_iterated, min, true, map_block, sum, sum_f);
		/*if(min == old_min)
		{
			min = LONG_MAX;
		}*/
	...
	//verify_checkpoint(map_methods, vector_items, it_start, count_iterated, LONG_MAX, false, map_block, sum, sum_f, sum_r);
	verify_checkpoint(map_methods, vector_items, it_start, count_iterated, LONG_MAX, false, map_block, sum, sum_f);
...
if (KFIFO_QUEUE) {
	...
	//std::string header_code = load_file("bench/kernel.h");
	
	// kernel calculates for each element C=A+B
	/*std::string kernel_code=
			"   void kernel simple_add(global double* sum, global double* sum_f, global double* sum_r, global unsigned long int* D){       "
			"		unsigned long int x = 1 << get_global_id(0);	"
			"		if(sum[get_global_id(0)] < 0) __sync_fetch_and_or (&D[0], x);	"
			"   }                                                                               ";
	*/
	/*std::string kernel_code=
			"   void kernel simple_add(global double* sum, global double* sum_f, global double* sum_r, global unsigned int* D){       "
			"		if(sum[get_global_id(0)] < 0) D[0] = 0;	"
			" 		if((ceil(sum[get_global_id(0)]) + sum_r[get_global_id(0)]) < 0) D[0] = 0;"
			" 		int N; "
			"		if(sum_f[get_global_id(0)] == 0) N = 0; else N = -1; "
			"		if(((ceil(sum[get_global_id(0)]) + sum_f[get_global_id(0)]) * N) < 0) D[0] = 0; "
			"   }                                                                               ";
	*/
	std::string kernel_code=
			//"   void kernel simple_add(global double* sum, global double* sum_f, global double* sum_r, global unsigned int* D){       "
			"   void kernel simple_add(global double* sum, global double* sum_f, global unsigned int* D){       "
			"		if(sum[get_global_id(0)] < 0) D[0] = 0;	"
			//" 		if((ceil(sum[get_global_id(0)]) + sum_r[get_global_id(0)]) < 0) D[0] = 0;"
			" 		int N; "
			"		if(sum_f[get_global_id(0)] == 0) N = 0; else N = -1; "
	...
	//std::string full_source = header_code + kernel_code;		
			
	sources.push_back({kernel_code.c_str(),kernel_code.length()});
	//sources.push_back({full_source.c_str(),full_source.length()});
	...
	cl::Buffer buffer_sum(context,CL_MEM_READ_WRITE,sizeof(double)*(TEST_SIZE*NUM_THRDS+1));
	cl::Buffer buffer_sum_f(context,CL_MEM_READ_WRITE,sizeof(double)*(TEST_SIZE*NUM_THRDS+1));
	//cl::Buffer buffer_sum_r(context,CL_MEM_READ_WRITE,sizeof(double)*(TEST_SIZE*NUM_THRDS+1));
	//cl::Buffer buffer_D(context,CL_MEM_READ_WRITE,sizeof(unsigned long int)*1);
	cl::Buffer buffer_D(context,CL_MEM_READ_WRITE,sizeof(unsigned int)*1);
 
	//create queue to which we will push commands for the device.
	cl::CommandQueue device_queue(context,default_device);
	
	buffer_sum_ptr = &buffer_sum;
	buffer_sum_f_ptr = &buffer_sum_f;
	//buffer_sum_r_ptr = &buffer_sum_r;
	buffer_D_ptr = &buffer_D;
	device_queue_ptr = &device_queue;
	program_ptr = &program;
